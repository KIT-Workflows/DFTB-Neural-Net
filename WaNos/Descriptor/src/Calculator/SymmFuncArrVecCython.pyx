...
import pandas as pd
import src.src_nogrd as src_nogrd
...
cpdef get_distance_tensor_rad(np.ndarray[NPdouble_t, ndim=3] distance_arr, np.ndarray[NPint_t, ndim=2] rad_arr, long n_atoms, long n_rads, long n_samples):

  cdef np.ndarray[NPdouble_t, ndim=1] Rij_rad = np.zeros([n_rads, n_samples], NPdouble)
...
cpdef get_distance_tensor(np.ndarray[NPdouble_t, ndim=2] distance_arr, np.ndarray[NPint_t, ndim=2] angle_arr, long n_atoms, long n_angs):
  ...
  cdef np.ndarray[NPdouble_t, ndim=1] Rij_vec = np.zeros( [n_angs, n_samples], NPdouble)
  cdef np.ndarray[NPdouble_t, ndim=1] Rik_vec = np.zeros( [n_angs, n_samples], NPdouble)
  cdef np.ndarray[NPdouble_t, ndim=1] Rjk_vec = np.zeros( [n_angs, n_samples], NPdouble)
...
cdef DIST distance_xyz_ind(x1, y1, z1, x2, y2, z2):
    ...
    return dist, dist_dx, dist_dy, dist_dz



cpdef distance_xyz_arr(np.ndarray[NPdouble_t, ndim=3] xyz_arr, long n_atoms, long n_samples):
    ...
    cdef np.ndarray[NPdouble_t, ndim=2] distance_arr = np.zeros( [n_dist, 4, n_samples], NPdouble )
    ...
    for at1 in range(0, n_atoms, 1):
        ...
        for at2 in range(at1+1, n_atoms, 1):
            ...
            dist, dist_dx, dist_dy, dist_dz = distance_xyz_ind(x1, y1, z1, x2, y2, z2)

            distance_arr[count, 0, :] = dist
            distance_arr[count, 1, :] = dist_dx
            distance_arr[count, 2, :] = dist_dy
            distance_arr[count, 3, :] = dist_dz
...
cpdef symm_func_mol_vec(np.ndarray[NPint_t, ndim=1] at_ele_arr,
                        ...
                        long n_atoms, long n_symm_func, long n_ele):
    """
    ...
    for at_idx_i in prange(0, n_atoms, 1, nogil=True):
        ...
        #for at_idx_j in neighbourlist_arr[at_idx_i]:
            ...
            for symm_count in range(symm_count_start, symm_count_end,1):
                rad_count = symm_count - symm_count_start
                #Gfunc_data[at_idx_i,0, symm_count] += radial_filter_ind(Rs, eta, Rij)
                Gfunc_data[at_idx_i, 0, symm_count] += rad_precalc[rad_count, ij_idx ]
        ...
        #for neighbour_pair in neighbourpair_arr[at_idx_i]:
            ...
            for symm_count in range(symm_count_start, symm_count_end , 1):
                ...
                Gfunc_data[at_idx_i, 0, symm_count] += ang_precalc[ang_count,  angular_idx]
...
cpdef symm_func_mol_vecidx(np.ndarray[NPint_t, ndim=1] at_ele_arr,
                        ...
                        np.ndarray[NPint_t, ndim=2] ele_count , np.ndarray[NPint_t , ndim=2] pair_count,
                        np.ndarray[NPdouble_t, ndim=3] rad_precalc, np.ndarray[NPdouble_t, ndim=3] ang_precalc,
                        long n_atoms, long n_symm_func, long n_ele, long n_rads, long n_angs, long rad_count_each, long ang_count_each, n_samples):
    """
    Refactor the entire symmetry function calculation into
    arr operations to speed up.
    ...
    cdef np.ndarray[NPdouble_t, ndim=3] Gfunc_data = np.zeros([n_atoms, n_samples, n_symm_func], NPdouble) # New Gfunc structure
    ...
    for rad_idx in range(0, n_rads, 1):
        ...
        for rad_count in range(0, rad_count_each, 1):
            ...
            Gfunc_data[at_i, :, symm_count_j] += rad_filter
            Gfunc_data[at_j, :, symm_count_i] += rad_filter
    ...
    for ang_idx in range(0, n_angs, 1):
        ...
        for symm_count in range(symm_count_start, symm_count_end, 1):
            ang_count = symm_count - symm_count_start
            Gfunc_data[at_i, :, symm_count] += ang_precalc[ang_count, ang_idx,:]
