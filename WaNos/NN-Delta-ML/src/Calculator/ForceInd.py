...
def calculate_analytical_force_ind_pile_compiled(nn_deriv_G_pile,
                                distance_arr, at_ele_arr, xyzArr,
    ...
    #Gfunc_ind  = Gfunc_data[at_b_idx][0:1,:]
    # nn_deriv_G = calculate_subnet_deriv_value(subnet_deriv_arr,
    #                                             at_b_ele, Gfunc_ind)
    G_deriv_xyz_pile = SymmDerivInd.symm_deriv_ind_pile_stacked(distance_arr, at_ele_arr, xyzArr,
                                neighbourlist_arr, neighbourlist_count,
                                neighbourpair_arr, neighbourpair_count,
                                count_Gparam,
                                ele_count, pair_count,
    ...
    atom_xyz = np.matmul(nn_deriv_G_pile, G_deriv_xyz_pile)
        ...
        #     # Profile Cython
        #     print("I am here")
        #     cProfile.runctx("SymmDerivInd.symm_deriv_ind_arr(distance_arr, at_ele_arr, xyzArr, neighbourlist_arr, neighbourpair_arr, count_Gparam, count_dict, ang_list, n_atoms, n_symm_func, at_b_idx, atom_ref_idx)",
        #     globals(), locals(), "Profile.prof")
        #     s = pstats.Stats("Profile.prof")
        #     s.strip_dirs().sort_stats("time").print_stats()
    # results = [proc_pool.map(get_dE_dref,
    #                                 subnet_deriv_arr, Gfunc_data, at_idx_map,
    #                                 Gparam_dict, atoms, atom_b, atom_ref_idx)
    #                                 for atom_b in atoms ]
    #outputs = [p.get() for p in results]
    #atom_xyz = np.sum(outputs, axis=1)
    return atom_xyz.reshape(3)


def calculate_analytical_force_mol_compiled(subnet_deriv_arr, Gfunc_data,
                                    distance_arr, at_ele_map, at_ele_arr, xyzArr,
                                    neighbourlist_arr, neighbourlist_count,
                                    neighbour_pair_arr, neighbourpair_count,
                                    count_Gparam, ele_count, pair_count, ang_precalc,
    ...
    atoms_force_xyz = np.zeros((n_atoms, 3), dtype=np.float32) # To contain force in xyz direction
...
def get_nn_deriv_G_pile_mol(subnet_deriv_arr, at_ele_arr,Gfunc_data, n_atoms, n_symm_func):
    ...
    nn_deriv_G_pile = np.empty( (1, n_symm_func * n_atoms))
    for at_idx in np.arange(n_atoms):
